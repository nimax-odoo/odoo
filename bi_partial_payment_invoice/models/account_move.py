...
class AccountMoveInherit(models.Model):
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        if self._context.get('partner_id') and self._context.get('type'):
            ...
            args += [('partner_id', '=', partner_id), ('move_type', '=', move_type), ('state', 'in', state), ('payment_state', 'in', payment_state)]
...
class AccountMoveLineInherit(models.Model):
    ...
    def _check_reconciliation(self):
        moves = self.mapped('move_id')
        for m in moves:
            m.payment_id.update({'state': 'posted'})
        for line in self:
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        if self._context.get('partner_id') and self._context.get('partner_type'):
            ...
            if self._context.get('partner_type') == 'customer':
                account_ids = self.env['account.account'].search([('company_id', '=', self.env.user.company_id.id), ('user_type_id.type', '=', 'receivable')])
                args.extend([('credit', '>', 0), ('debit', '=', 0), ('account_id', 'in', account_ids.ids or []), ('partner_id', '=', partner_id)])
            else:
                account_ids = self.env['account.account'].search([('company_id', '=', self.env.user.company_id.id), ('user_type_id.type', '=', 'payable')])
