...
class website(models.Model):
    ...
    def get_dynamic_count(self, product_ids):
...
class ProductFilters(models.Model):
    ...
    group_id = fields.Many2one('group.filter', 'Group Filter', required=True,
                               default=lambda self: self.env['group.filter'].sudo().search([('name', '=', 'Other Filters')],
                                                                                    limit=1))
...
class FilterProductValue(models.Model):
    ...
    def _attribute_count(self):
        for filters in self:
            ...
            product_ids = product_obj.sudo().search([])
            count = 0
            if product_ids:
