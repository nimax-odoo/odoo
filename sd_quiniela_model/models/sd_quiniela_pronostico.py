...
class SdQuinielaData(models.Model):
    ...
    datas_ids = fields.One2many(
        'sd.quiniela.data',
        'quiniela_pronostico_id',
        string='Quiniela Partidos'
    )
    ...
    def action_pronosticos_partidos(self):
        for record in self:
            ...
            record.datas_ids.unlink()

            pronosticos = self.env['sd.quiniela.partidos'].sudo().search([
                ('active', '=', True)
            ])

            if not pronosticos:
                raise UserError(_('No existen partidos activos para generar pronósticos.'))
